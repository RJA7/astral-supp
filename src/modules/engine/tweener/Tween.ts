...
import type Tweener from './Tweener';
...
export class Tween<T extends UnknownProps = any> {
	...
	start(
		time: number = this._group.getTime(),
		...
		for (const property in _valuesEnd) {
			...
			if (isInterpolationList) {
				...
				for (let i = 0, l = endValues.length; i < l; i += 1) {
					const value = this._handleRelativeValue(
						startValue as number,
						endValues[i],
					);
					if (isNaN(value)) {
						isInterpolationList = false;
						print('Found invalid interpolation list. Skipping.');
						break;
					}
					temp.push(value);
				}
	...
	pause(time: number = this._group.getTime()): this {
	...
	resume(time: number = this._group.getTime()): this {
		if (!this._isPaused || !this._isPlaying) {
			return this;
		}
	...
	update(
		...
		const durationAndDelay =
			this._duration + (this._repeatDelayTime ?? this._delayTime);
		...
		const calculateElapsedPortion = () => {
			...
			const timeIntoCurrentRepeat =
				elapsedTime - timesRepeated * durationAndDelay;
			// TODO use %?
